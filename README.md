...
<p>Клонируем репозиторий:
    git clone https://github.com/danognev/TCPChat</p>
<p>Переходим в директорию со скачанным репозиторием:
    cd TCPChat</p>
<p>Запускаем скрипт для сборки проекта:
    bash BUILD_PROJECT.sh</p>
...
<p>Для запуска сервера или модульных тестов необходимо перейти в директорию с испольняемыми файлами:
    cd build/bin</p>
<p>Затем для запуска сервера необходимо ввести:
    ./Server [порт]</p>
<p>Для запуска модульных тестов: 
    ./UnitTests</p>
<h2 id="-">Подключение к серверу</h2>
<p>После запуска сервера, например, <code>./Server 3333</code>, мы можем подключиться к нему используя, как вариант, утилиту telnet. Для этого необходимо прописать:
    telnet [ip адрес сервера] [порт]</p>
<p>Вы успешно подключились к чату!
</a>
</a></p>
